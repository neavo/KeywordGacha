...
- 本地与在线接口均可，KG 的开发环境是本地的 [Qwen2-7B](https://huggingface.co/Qwen/Qwen2-7B-Instruct-GGUF)
...
- 如果你有一台拥有至少8G显存的Nvidia显卡的电脑，可以使用由作者提供的 [KeywordGachaServer 一键包](https://github.com/neavo/KeywordGachaServer)

### 文本格式 🆗
- 目前支持对两种文本格式的解析，抓取效果不好时，可以两种都试试
- 如当前目录下有 `all.orig.txt` 或 `ManualTransFile.json` 文件，会自动识别
- 文件中 一行/一条 应包含一个句子，太长的话请先手动处理一下，会撑爆 AI 的上下文缓存导致灵异问题
